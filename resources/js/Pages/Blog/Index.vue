                ...
                <main class="mt-6">
                    ...
                    <div v-if="posts && posts.data.length">
                        <div v-for="post in posts.data" :key="post.id"
                            class="mb-6 border p-6 rounded-lg bg-white dark:bg-gray-800 shadow-sm hover:shadow-md transition-shadow">
                            <div class="flex items-start gap-4">
                                <template v-if="post.thumbnail">
                                    <img :src="post.thumbnail" alt="thumbnail"
                                        class="hidden sm:block w-24 h-24 rounded-md object-cover shrink-0" />
                                </template>

                                <div class="flex-1 min-w-0">
                                    <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200">
                                        <Link :href="route('blog.show', post.slug)" class="block truncate">{{ post.title
                                        }}</Link>
                                    </h2>

                                    <p class="text-sm text-gray-500 dark:text-gray-400">Published: {{ post.published_at
                                    ...
                                    <p class="mt-2 text-gray-500 dark:text-gray-400">
                                        ...
                                        <template v-if="post.has_more">
                                            <Link :href="route('blog.show', post.slug)"
                                                class="text-sm text-gray-500 dark:text-gray-400 underline ms-2">Read
                                            More</Link>
                                        </template>
                                    </p>
                                </div>
