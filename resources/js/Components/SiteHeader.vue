...
<template>
    <header class="relative">
        <!-- Desktop Header -->
        <div class="hidden lg:grid lg:grid-cols-3 lg:items-center lg:gap-2 lg:py-10">
            <Link href="/" class="flex justify-left col-start-1">
            ...
            <nav class="-mx-3 flex items-center gap-2 justify-end justify-self-end col-start-3">
                ...
                <div class="relative" @mouseleave="closeAboutDropdown">
                    <button @click="toggleAboutDropdown" @mouseenter="aboutDropdownOpen = true"
                    ...
                    <div v-show="aboutDropdownOpen"
                        class="absolute right-0 w-56 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 z-50">
                        <div class="py-1">
                            <DropdownLink
                            ...
                            <DropdownLink
                                :href="$page.props.aboutSettings?.fanbase_slug ? route('about.fanbase', { slug: $page.props.aboutSettings.fanbase_slug }) : route('about.fanbase')"
                                @click="closeAboutDropdown"
                                class="block px-4 py-2 text-md text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
                ...
                <Link v-for="page in $page.props.menuPages" :key="page.slug" :href="`/${page.slug}`"
                    class="rounded-md px-3 py-2 text-black ring-1 ring-transparent transition hover:text-black/70 focus:outline-none focus-visible:ring-[#FF2D20] dark:text-white dark:hover:text-white/80 dark:focus-visible:ring-white">
                    {{ page.title }}</Link>
        ...
        <!-- Mobile Header -->
        <div class="lg:hidden flex items-center justify-between py-6 px-4">
            <Link href="/" class="flex">
                <img v-if="$page.props.appSettings?.app_logo" :src="$page.props.appSettings.app_logo"
                    class="h-10 w-auto object-contain" alt="logo" />
                <template v-else>
                    <img src="/storage/logo.svg" class="h-10 w-auto object-contain" alt="default logo" />
                </template>
            </Link>
        ...
        <!-- Mobile Menu Overlay -->
        <div v-if="mobileMenuOpen" class="lg:hidden block sm:hidden">
            <div class="pt-2 pb-1 border-t border-gray-200 dark:border-gray-600">
                <div class="mt-3 space-y-1 px-4">
                    <Link v-if="$page.props.auth.user" :href="route('dashboard')" @click="closeMobileMenu"
                        class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:text-gray-900 dark:focus:text-white focus:bg-gray-50 dark:focus:bg-gray-700 transition ease-in-out duration-150">
                        Dashboard
                    </Link>

                    <Link :href="route('blog.index')" @click="closeMobileMenu"
                        class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:text-gray-900 dark:focus:text-white focus:bg-gray-50 dark:focus:bg-gray-700 transition ease-in-out duration-150">
                        Blog
                    </Link>

                    <Link :href="route('public.gallery')" @click="closeMobileMenu"
                        class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:text-gray-900 dark:focus:text-white focus:bg-gray-50 dark:focus:bg-gray-700 transition ease-in-out duration-150">
                        Gallery
                    </Link>

                    <!-- Custom Pages -->
                    <div v-if="$page.props.menuPages && $page.props.menuPages.length" class="space-y-1">
                        <Link v-for="page in $page.props.menuPages" :key="page.slug" :href="`/${page.slug}`"
                    ...
                    <!-- About Section -->
                    <div class="pt-4 border-t border-gray-200 dark:border-gray-600 mt-4 space-y-1">
                    ...
                    <!-- Social Media -->
                    <div v-if="$page.props.appSettings?.instagram_url || $page.props.appSettings?.twitter_url || $page.props.appSettings?.tiktok_url"
                        class="py-4 border-t border-gray-200 dark:border-gray-600 mt-4 space-y-1">
                        <h3 class="px-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                            Follow Us
                        </h3>
                        <div class="flex gap-4 px-3 py-2">
                            <a v-if="$page.props.appSettings?.instagram_url"
                                :href="$page.props.appSettings.instagram_url" target="_blank" rel="noopener noreferrer"
                                class="text-gray-600 dark:text-gray-400 hover:text-pink-600 dark:hover:text-pink-400 transition">
                            ...
                            <a v-if="$page.props.appSettings?.tiktok_url" :href="$page.props.appSettings.tiktok_url"
                                target="_blank" rel="noopener noreferrer"
