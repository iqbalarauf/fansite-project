    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('show-teater.store') }}" method="POST" class="space-y-6">
                    @csrf

                    <div class="grid gap-4 lg:grid-cols-2">
                        <div>
                            <label for="show_id" class="block text-sm font-medium text-gray-700">Show ID</label>
                            <input type="number" name="show_id" id="show_id" value="{{ old('show_id', $nextShowId) }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required />
                        </div>

                        <div>
                            <label for="show_date" class="block text-sm font-medium text-gray-700">Tanggal</label>
                            <input type="date" name="show_date" id="show_date" value="{{ old('show_date') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required />
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <select name="setlist" id="setlist"
                                ...
                                @foreach ($setlistsWithUnitSongs as $setlist)
                                    <option value="{{ $setlist['name'] }}" {{ old('setlist') === $setlist['name'] ? 'selected' : '' }}>{{ $setlist['display_name'] }}</option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select name="unit_song" id="unit_song"
                                ...
                                @foreach ($setlistsWithUnitSongs as $setlist)
                                    @foreach ($setlist['unit_songs'] as $song)
                                        <option value="{{ $song['name'] }}" {{ old('unit_song') === $song['name'] ? 'selected' : '' }}>{{ $setlist['display_name'] }} - {{ $song['display_name'] }}</option>
                                    @endforeach
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="grid gap-4 lg:grid-cols-2">
                        <div class="flex items-center gap-3">
                            <input type="checkbox" name="is_global_center" id="is_global_center" value="1" {{ old('is_global_center') ? 'checked' : '' }}
                                class="h-4 w-4 text-indigo-600 border-gray-300 rounded" />
                            <label for="is_global_center" class="text-sm font-medium text-gray-700">Global Center</label>
                        </div>

                        <div class="flex items-center gap-3">
                            <input type="checkbox" name="is_us_center" id="is_us_center" value="1" {{ old('is_us_center') ? 'checked' : '' }}
                                class="h-4 w-4 text-indigo-600 border-gray-300 rounded" />
                    ...
                    <div>
                        ...
                        <input type="text" name="is_the_show_has_event" id="is_the_show_has_event" value="{{ old('is_the_show_has_event') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="additional_information" id="additional_information" value="{{ old('additional_information') }}"
                    ...
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
